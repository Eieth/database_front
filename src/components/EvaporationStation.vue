...
axios.post(baseURL + '/database/delete' + 'EvaporationStation',
    {
...
let filterTableData = computed(() => {
    if (dialogStatus.value === 0){
        ...
        Object.keys(EvaporationStationForm.value).forEach(key => {
            if (nullObjectHandler(EvaporationStationForm.value[key]) === '') {
                return;
            }
            res = res.filter((data)=> {
                return nullObjectHandler(data[key]).toString().includes(EvaporationStationForm.value[key]);
            })
        });
        return res;
    }
