...
let filterTableData = computed(() => {
    if (dialogStatus.value === 0) {
        ...
        Object.keys(DynamicInfo.value).forEach(key => {
            if (nullObjectHandler(DynamicInfo.value[key]) === '') {
                return;
            }
            res = res.filter((data)=> {
                return nullObjectHandler(data[key]).toString().includes(DynamicInfo.value[key]);
            })
        });
        return res;
    }
...
axios.post(baseURL + '/database/delete' + 'DynamicInfo',
    {
